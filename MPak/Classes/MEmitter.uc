...
class MEmitter extends Emitter
	Config(MPak);


var(Emitter) array<float> fOpacities;
...
// Emitter
var(Emitter) bool _TriggerToggleSpawning;
...
event PostBeginPlay()
{
	...
	for(i = 0; i < fOpacities.Length; i++)
	{
		Emitters[i].SetPropertyText("Opacity", string(fOpacities[i]));
	}
...
function HackVariables()
{
	...
	SetPropertyText("TriggerToggleSpawning", U.BoolToString(_TriggerToggleSpawning));
}
