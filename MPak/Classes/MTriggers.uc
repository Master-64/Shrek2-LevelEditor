...
// *****************************************************
// 
// Categorizes all of the Triggers
...
// Trigger
var(Trigger) bool _bTT_CP_EventMustMatchTag;
var(Trigger) bool _bActivateOnUntouchInsteadOfTouch;
var(Trigger) bool _bTriggerByDirection;
var(Trigger) KWTrigger.ETellOtherTrigger _TellOtherTrigger;
...
{
	SetPropertyText("bTT_CP_EventMustMatchTag", U.BoolToString(_bTT_CP_EventMustMatchTag));
	SetPropertyText("bActivateOnUntouchInsteadOfTouch", U.BoolToString(_bActivateOnUntouchInsteadOfTouch));
	SetPropertyText("bTriggerByDirection", U.BoolToString(_bTriggerByDirection));
	SetPropertyText("TellOtherTrigger", string(_TellOtherTrigger));
...
function Activate(Actor Other, Pawn Instigator) // Partial port of KnowWonder's Activate() function
{
	local Actor A;
	local int i;
	...
	if(Message != "")
	{
		U.CM((string(Name) $ " Message: ") $ Message);
	}
}
